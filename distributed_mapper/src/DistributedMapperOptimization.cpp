...
int main(int argc, char* argv[])
{
    ...
    // 载入各个子地图位姿态，存储到distMappers中去，之后作为分布式算法的输入
    for(size_t robot = 0; robot < nrRobots; robot++){
    ...
    // 建立的图是联合图,进行的分布式优化的步骤
    if(!disconnectedGraph){
        try{
            /* try optimizing
             * @brief 分布优化(DGS)进行估计并返回计算结果
             * @param updateType:采用的更新方式：Distributed Jacobi/Jacobi OverRelaxation (postUpdate) 还是 Gauss-Seidel/Successive OverRelaxation (incUpdate)
             * @param gamma:over relaxation methods中的gamma值
             * @param rotationEstimateChangeThreshold:终止条件
             * @param poseEstimateChangeThreshold:Difference between pose estimate provides an early stopping condition
            ...
            */
            ...
            // 将所有机器人的计算结果整合到一起,将估计从vector<Values>格式转换为Values格式
            Values distributed;
            ...
            // @param fullGraph:graph NonlinearFactor graph storing the measurements
            // @param distributed:graph ValueS storing the values
