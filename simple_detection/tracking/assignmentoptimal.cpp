...
{
	int N=DistMatrix.size(); // êîë-âî ñòîëáöîâ tracks
	int M=DistMatrix[0].size(); // êîë-âî ñòðîê measurements

	int *assignment		=new int[N];
	double *distIn		=new double[N*M];

	double  cost;
	// Çàïîëíÿåì ìàòðèöó ñëó÷àéíûìè ÷èñëàìè
	for(int i=0; i<N; i++)
	{
		for(int j=0; j<M; j++)
		{
...
void AssignmentProblemSolver::assignmentoptimal(int *assignment, double *cost, double *distMatrixIn, int nOfRows, int nOfColumns)
{
	...
	*cost = 0;
	...
	// Ãåíåðèðóåì ðàáî÷óþ êîïèþ ìàòðèöû ðàññòîÿíèé 
	// è ïðîâåðêà ýëåìåíòîâ ìàòðèöû íà ïîçèòèâíîñòü :)

	// Îáùåå êîëè÷åñòâî ýëåìåíòîâ
	nOfElements   = nOfRows * nOfColumns; 
	// Âûäåëÿåì ïàìÿòü
	distMatrix    = (double *)malloc(nOfElements * sizeof(double));
	// Óêàçàòåëü íà ïîñëåäíèé ýëåìåíò
	distMatrixEnd = distMatrix + nOfElements;

	// Öèêë ïðîâåðêè è ïðèñâîåíèÿ
